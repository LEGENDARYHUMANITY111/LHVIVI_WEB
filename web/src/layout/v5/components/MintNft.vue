        ...
        const batchMintNft = async (collectModel: any) => {
            ...
            try {
                ...
                MessageArrayExpose.value.push({
                    message: 'You want mint ' + collectModel.mint_amount + ' NFTs, specify NftId: ' + (collectModel?.nft_id||0),
                })
                ...
                if ("id" in collectModel && collectModel.summary_status == 1) {
                    ...
                    // 取实际能mint的NFT数据下来
                    let batchMintInfos = await getBatchMintNftsByCid({ collection_id: collectModel.id, count: preMintAmount,nft_id:collectModel?.nft_id||0 });
